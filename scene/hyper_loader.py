...
class Load_hyper_data(Dataset):
    ...
    def __getitem__(self, index):
        ...
        elif self.split == "test_bouncingballs.py":
    ...
    def load_raw(self, idx):
        ...
        image_path = "/".join(self.all_img[idx].split("/")[:-1])
        image_name = self.all_img[idx].split("/")[-1]
        if self.image_mask is not None and self.split == "test_bouncingballs.py":
...
def format_hyper_data(data_class, split):
    ...
    elif split == "test_bouncingballs.py":
    ...
    for uid, index in tqdm(enumerate(data_idx)):
        ...
        if data_class.image_mask is not None and data_class.split == "test_bouncingballs.py":
